...
function setup() {
  const container = select('#p5-container-GWsineGen');
  if (!container) {
    console.error("Kontener #p5-container-GWsineGen nie został znaleziony.");
    return;
  }
  ...
  tSlider = createSlider(0.05, 5, 4.5, 0.05); // Min: 0.1, Max: 5, Start: 0.5, Krok: 0.1
...
function computeTtf(x, t) {
  ...
  return (integral -sinFunction(x))/t;
}
